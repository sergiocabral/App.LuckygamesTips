...
class EntidadeBdLicenca extends EntidadeBd {
  ...
  UNIQUE KEY `un_lice_email` (`email`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='Licenças concedidas para módulos do Glasgow.';");
    ...
    public static function CriarDadosIniciais(BancoConexaoMySQL $bd = null) {
        if (count((new EntidadeBdLicenca($bd))->Listar()) === 0) {
            $padrao = [
                [
                    'nome' => 'Fãs da Glasgow',
                    'email' => 'glasgow@splitz.com.br',
                    'validade' => null,
                    'permanente' => 0,
                    'modulos' => 'Estatisticas Limites TrocaSeed',
                ...
                [
                    ...
                    'validade' => null,
                    'permanente' => 1,
                    'modulos' => 'Estatisticas Limites Espera TrocaSeed Martin Nainei',
    ...
    public function Inserir(BancoConexaoMySQL $bd = null) {
        ...
        $prepare = $bd->ExecutarSQL(
            "INSERT INTO `licenca` (`nome`, `email`, `validade`, `permanente`, `modulos`) VALUE (?, ?, ?, ?, ?)", true,
            "sssis", [ &$this->nome, &$this->email, &$this->validade, &$this->permanente, &$this->modulos ]);
    ...
    public function Alterar(BancoConexaoMySQL $bd = null) {
        ...
        if ($this->VerificarId()) {
            $prepare = $bd->ExecutarSQL(
                "UPDATE `licenca` set `nome` = ?, `email` = ?, `validade` = ?, `permanente` = ?, `modulos` = ? WHERE `id` = ?", true,
                "sssisi", [ &$this->nome, &$this->email, &$this->validade, &$this->permanente, &$this->modulos, &$this->id]);
        }
